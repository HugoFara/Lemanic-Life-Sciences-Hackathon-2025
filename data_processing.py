...
for coder in ["coder1", "coder2"]:
    ...
    for sentence in labeled_phonemes:
        ...
        ipa_words = [ipa_encoder.get_ipa(word, language) for word in words]
        ipa_output = ["".join(item for sublist in ipa_encoder.get_ipa(word, language) for item in sublist) for word in words]
